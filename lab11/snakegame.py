...
try:
    ...
    gameLoop()
    cur.execute('''SELECT 1 FROM snakegame WHERE username = %s''', (user1,))
    exists = cur.fetchone()
    if exists:
        cur.execute('''UPDATE snakegame
                        SET level = %s
                        WHERE username = %s''', (USER_LEVEL, user1))
    else:
        cur.execute('''INSERT INTO snakegame (username, level)
                           VALUES (%s, %s)''', (user1, USER_LEVEL))
